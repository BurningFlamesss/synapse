...
class HookSystem:
    ...
    def _build_env(self, trigger: HookTrigger, tool_name: str | None = None, user_message: str | None = None, error: Exception | None = None) -> dict[str, str]:
        ...
        env = os.environ.copy()
        env["SYNAPSE_TRIGGER"] = trigger.value
        env["SYNAPSE_CWD"] = str(self.config.cwd)

        if tool_name:
            env["SYNAPSE_TOOL_NAME"] = tool_name

        if user_message:
            env["SYNAPSE_USER_MESSAGE"] = user_message

        if error:
            env["SYNAPSE_ERROR"] = str(error)
    ...
    async def trigger_after_agent(
        ...
        env["SYNAPSE_RESPONSE"] = agent_response
    ...
    async def trigger_before_tool(
        ...
        env["SYNAPSE_TOOL_PARAMS"] = json.dumps(tool_params)
    ...
    async def trigger_after_tool(
        ...
        env["SYNAPSE_TOOL_PARAMS"] = json.dumps(tool_params)
        env["SYNAPSE_TOOL_RESULT"] = tool_result.to_model_output()
